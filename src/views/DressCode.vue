<template>
<WeddingCard title ="Dress Code">
        <section class="dresscode">
  <p class="intro-text">
    Afin de mettre subtilement à l'honneur notre lien commun avec le karaté, 
    nous avons associé chaque couleur de ceinture à une durée de couple.
  </p>

  <div class="dresscode-grid">
    <div class="color-item" v-for="belt in belts" :key="belt.color">
      <div class="belt-preview" :class="belt.class"></div>
      <div class="belt-info">
        <span class="belt-name">{{ belt.label }} :</span>
        <span class="belt-desc">{{ belt.desc }}</span>
      </div>
    </div>
  </div>

  <div class="example-box">
    <p><strong>Exemple :</strong> Si Baltazar est en couple depuis 46 ans, il pourra porter une pochette de costume rouge.</p>
  </div>
</section>
</WeddingCard>
</template>
...
.dresscode-grid {
  display: grid;
  grid-template-columns: repeat(2, 1fr); /* Deux colonnes pour la lisibilité */
  gap: 20px;
  max-width: 600px;
  margin: 30px auto;
}
...
.belt-preview {
  width: 40px;
  height: 12px; /* Forme de barrette/ceinture */
  border-radius: 3px;
  margin-right: 15px;
